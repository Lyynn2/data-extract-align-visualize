...
int ecg_adc_setup()
{
  ...
  // Connect to the ADC.
  ecg_adc_i2c_device = i2cOpen(ECG_I2C_BUS, ECG_ADC_I2C_ADDRESS, 0);
  #if ECG_ADC_DEBUG_PRINTOUTS
  printf("ADC device index: %d\n", ecg_adc_i2c_device);
  #endif
  if(ecg_adc_i2c_device < 0)
  {
    ECG_LOG("ecg_adc_setup(): Failed to connect to the ADC: returned %d.", ecg_adc_i2c_device);
  ...
  ECG_LOG("ecg_adc_setup(): ADC connected successfully!\n");
...
{
  ECG_LOG("ecg_adc_cleanup(): Terminating the GPIO interface.\n");
  gpioTerminate();
}
...
// @param rate Can be E20, 90, 330, or 1000.
...
uint8_t ecg_adc_read_register(uint8_t reg)
{
  ...
  if(result == PI_BAD_HANDLE || result == PI_I2C_READ_FAILED)
    ECG_LOG("ecg_adc_read_register(): Failed to read the desired register.\n");
  return result;
}
